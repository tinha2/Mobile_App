...
//        let headerWithKeyId = JWTHeader()
    ...
    static func getSentimentEntities(withContent content:String) -> Observable<Response> {
        ...
        if !LocalCoordinator.share.isExpireToken {
            return requestToAnalysisTextWithKey(endpoint: analyzeAnnotateText, content: content, feature: features)
                .map { Response($0) }
        }
    ...
    static func synthesiseVoice(withContent voiceBase64:String,sampleRateHertz:Int,fileExtension:String) -> Observable<Response> {
        ...
        if !LocalCoordinator.share.isExpireToken {
            return requestToRecognizeVoiceWithKey(endpoint: recognizeSpeech, content: voiceBase64, configs: config)
                        .map { Response($0) }
        }
    ...
    static func extractImage(withContent imageBase64:String) -> Observable<Response> {
        ...
        if !LocalCoordinator.share.isExpireToken {
            return requestToExtractImageWithKey(endpoint: extractAnnotateImage, content: imageBase64, features: features)
                    .map { Response($0) }
        }
    ...
    static func synthesisText(withText text:String) -> Observable<Response> {
        ...
        if !LocalCoordinator.share.isExpireToken {
            return requestToSynthesisTextWithKey(content: text)
                    .map { Response($0) }
        }
    ...
    static func requestToAnalysisTextWithKey(endpoint: String, content:String, feature:([String:Bool])? = nil) -> Observable<[String: Any]> {
        var bodyRequest:[String:Any] = [:]
        bodyRequest["encodingType"] = "UTF8"
        bodyRequest["document"] = [
            "type":"PLAIN_TEXT",
        ...
        let queryParam:[String:Any] = ["key": Google_Cloud_Key]
...
//            "Authorization":"Bearer \(LocalCoordinator.share.accessToken)"
    ...
    static func requestToExtractImageWithKey(endpoint: String, content:String, features:[String] = []) -> Observable<[String: Any]> {
        var bodyRequest:[String:Any] = [:]
    ...
    static func requestToSynthesisTextWithKey(content: String) -> Observable<[String: Any]> {
    ...
    static func requestToRecognizeVoiceWithKey(endpoint: String, content:String, configs:CommonDic) -> Observable<[String: Any]> {
        var bodyRequest:[String:Any] = [:]
